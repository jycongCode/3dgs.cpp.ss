...
void Renderer::createPrefixSumPipeline() {
	...
	auto tileX = (width + 16 - 1) / 16;
	auto tileY = (height + 16 - 1) / 16;
    if (configuration.stochasticSplatting) {
        prefixSumPingBuffer = Buffer::storage(context, tileX * tileY * sizeof(uint32_t), false);
        prefixSumPongBuffer = Buffer::storage(context, tileX * tileY * sizeof(uint32_t), false);
    }
    else {
...
void Renderer::recordPreprocessCommandBuffer() {
	...
	auto tileX = (width + 16 - 1) / 16;
	auto tileY = (height + 16 - 1) / 16;
	numGroups = (tileX * tileY + 255) / 256;
    const auto iters = static_cast<uint32_t>(std::ceil(std::log2(static_cast<float>(tileX * tileY))));
    ...
    vk::BufferCopy totalSumRegion,prefixSumRegion;
    if (configuration.stochasticSplatting) {
       totalSumRegion = vk::BufferCopy{ (tileX * tileY - 1) * sizeof(uint32_t), 0, sizeof(uint32_t) };
	   prefixSumRegion = vk::BufferCopy{ 0, 0, tileX * tileY * sizeof(uint32_t) };
    }
    else {
		totalSumRegion = vk::BufferCopy{ (scene->getNumVertices()-1) * sizeof(uint32_t), 0, sizeof(uint32_t)};
    }
    if (iters % 2 == 0) {
...
// RendererST
RendererST::~RendererST() {
}

void RendererST::initialize() {
    initializeVulkan();
    createGui();
    loadSceneToGPU();
    createPreprocessPipeline_ST();
    createPrefixSumPipeline();
    createProcessInstancesPipeline_ST();
    createTileBoundaryPipeline_ST();
    createRenderPipeline_ST();
    createCommandPool();
    recordPreprocessCommandBuffer_ST();
}

void RendererST::recreateSwapchain() {
    auto oldExtent = swapchain->swapchainExtent;
    spdlog::debug("Recreating swapchain");
    swapchain->recreate();
    if (swapchain->swapchainExtent == oldExtent) {
    ...
    auto tileX = (width + 16 - 1) / 16;
    auto tileY = (height + 16 - 1) / 16;
    tileOverlapBuffer_ST->realloc(tileX * tileY * sizeof(uint32_t));
    tileBoundaryBuffer_ST->realloc(tileX * tileY * sizeof(uint32_t) * 2);
    prefixBuffer_ST->realloc(tileX * tileY * sizeof(uint32_t));
    instanceCounterBuffer_ST->realloc(tileX * tileY * sizeof(uint32_t));
	prefixSumPingBuffer->realloc(tileX * tileY * sizeof(uint32_t));
	prefixSumPongBuffer->realloc(tileX * tileY * sizeof(uint32_t));
    recordPreprocessCommandBuffer_ST();
    createRenderPipeline_ST();
}

void RendererST::draw() {
    auto ret = context->device->waitForFences(inflightFences[0].get(), VK_TRUE, UINT64_MAX);
    if (ret != vk::Result::eSuccess) {
        throw std::runtime_error("Failed to wait for fence");
    }
...
startOfRenderLoopST:
    ...
    auto submitInfo = vk::SubmitInfo{}.setCommandBuffers(preprocessCommandBuffer_ST.get());
    context->queues[VulkanContext::Queue::COMPUTE].queue.submit(submitInfo, inflightFences[0].get());
    ...
    if (!recordRenderCommandBuffer_ST(0)) {
        goto startOfRenderLoopST;
    }
    vk::PipelineStageFlags waitStage = vk::PipelineStageFlagBits::eComputeShader;
    submitInfo = vk::SubmitInfo{}.setWaitSemaphores(swapchain->imageAvailableSemaphores[0].get())
        .setCommandBuffers(renderCommandBuffer_ST.get())
        .setSignalSemaphores(renderFinishedSemaphores[0].get())
        .setWaitDstStageMask(waitStage);
    context->queues[VulkanContext::Queue::COMPUTE].queue.submit(submitInfo, inflightFences[0].get());
...
void RendererST::createPreprocessPipeline_ST() {
    spdlog::debug("Creating preprocess pipeline for stochastic splatting");
    uniformBuffer = Buffer::uniform(context, sizeof(UniformBuffer));
    vertexAttributeBuffer = Buffer::storage(context, scene->getNumVertices() * sizeof(VertexAttributeBuffer), false);
    auto [width, height] = swapchain->swapchainExtent;
    auto tileX = (width + 16 - 1) / 16;
    ...
    tileOverlapBuffer_ST = Buffer::storage(context, tileX * tileY * sizeof(uint32_t), false);

    preprocessPipeline_ST = std::make_shared<ComputePipeline>(
        context, std::make_shared<Shader>(context, "preprocess_st", SPV_PREPROCESS_ST, SPV_PREPROCESS_ST_len));
    inputSet = std::make_shared<DescriptorSet>(context, FRAMES_IN_FLIGHT);
    inputSet->bindBufferToDescriptorSet(0, vk::DescriptorType::eStorageBuffer, vk::ShaderStageFlagBits::eCompute,
        scene->vertexBuffer);
    inputSet->bindBufferToDescriptorSet(1, vk::DescriptorType::eStorageBuffer, vk::ShaderStageFlagBits::eCompute,
        scene->cov3DBuffer);
    inputSet->build();
    preprocessPipeline_ST->addDescriptorSet(0, inputSet);
    ...
    uniformOutputSet->bindBufferToDescriptorSet(2, vk::DescriptorType::eStorageBuffer,
        vk::ShaderStageFlagBits::eCompute,
        tileOverlapBuffer_ST);
    uniformOutputSet->build();

    preprocessPipeline_ST->addDescriptorSet(1, uniformOutputSet);
    preprocessPipeline_ST->build();
}

void RendererST::createProcessInstancesPipeline_ST() {
    spdlog::debug("Creating process instances pipeline for stochastic splatting");
    auto [width, height] = swapchain->swapchainExtent;
    int tileX = (width + 15) / 16;
    int tileY = (height + 15) / 16;

    instanceIndexBuffer_ST = Buffer::storage(context, tileX * tileY * perTileSplatMultiplier * sizeof(uint32_t), false);
    instanceCounterBuffer_ST = Buffer::storage(context, tileX * tileY * sizeof(uint32_t), false);
    prefixBuffer_ST = Buffer::storage(context, tileX * tileY * sizeof(uint32_t), false);
    processInstancesPipeline_ST = std::make_shared<ComputePipeline>(
        context, std::make_shared<Shader>(context, "process_instances_st", SPV_PROCESS_INSTANCES_ST, SPV_PROCESS_INSTANCES_ST_len));

    auto descriptorSet1 = std::make_shared<DescriptorSet>(context, FRAMES_IN_FLIGHT);
    descriptorSet1->bindBufferToDescriptorSet(0, vk::DescriptorType::eStorageBuffer, vk::ShaderStageFlagBits::eCompute,
        vertexAttributeBuffer);
    descriptorSet1->bindBufferToDescriptorSet(1, vk::DescriptorType::eUniformBuffer, vk::ShaderStageFlagBits::eCompute,
        uniformBuffer);
    descriptorSet1->bindBufferToDescriptorSet(2, vk::DescriptorType::eStorageBuffer, vk::ShaderStageFlagBits::eCompute,
        prefixBuffer_ST);
    descriptorSet1->build();

    auto descriptorSet2 = std::make_shared<DescriptorSet>(context, FRAMES_IN_FLIGHT);
    descriptorSet2->bindBufferToDescriptorSet(0, vk::DescriptorType::eStorageBuffer, vk::ShaderStageFlagBits::eCompute,
        instanceIndexBuffer_ST);
    descriptorSet2->bindBufferToDescriptorSet(1, vk::DescriptorType::eStorageBuffer, vk::ShaderStageFlagBits::eCompute,
        instanceCounterBuffer_ST);

    descriptorSet2->build();
    processInstancesPipeline_ST->addDescriptorSet(0, descriptorSet1);
    processInstancesPipeline_ST->addDescriptorSet(1, descriptorSet2);
    processInstancesPipeline_ST->build();
}

void RendererST::createTileBoundaryPipeline_ST() {
    spdlog::debug("Creating tile boundary pipeline for stochastic splatting");
    auto [width, height] = swapchain->swapchainExtent;
    auto tileX = (width + 16 - 1) / 16;
    auto tileY = (height + 16 - 1) / 16;
    tileBoundaryBuffer_ST = Buffer::storage(context, tileX * tileY * sizeof(uint32_t) * 2, false);
    tileBoundaryPipeline_ST = std::make_shared<ComputePipeline>(
        context, std::make_shared<Shader>(context, "tile_boundary_st", SPV_TILE_BOUNDARY_ST, SPV_TILE_BOUNDARY_ST_len));

    auto descriptorSet = std::make_shared<DescriptorSet>(context, FRAMES_IN_FLIGHT);
    descriptorSet->bindBufferToDescriptorSet(0, vk::DescriptorType::eStorageBuffer, vk::ShaderStageFlagBits::eCompute,
        prefixBuffer_ST);
    descriptorSet->bindBufferToDescriptorSet(1, vk::DescriptorType::eStorageBuffer, vk::ShaderStageFlagBits::eCompute,
        tileBoundaryBuffer_ST);
    descriptorSet->build();

    tileBoundaryPipeline_ST->addDescriptorSet(0, descriptorSet);
    tileBoundaryPipeline_ST->addPushConstant(vk::ShaderStageFlagBits::eCompute, 0, sizeof(uint32_t) * 2);
    tileBoundaryPipeline_ST->build();
}

void RendererST::createRenderPipeline_ST() {
    spdlog::debug("Creating render pipeline for stochastic splatting");
    renderPipeline_ST = std::make_shared<ComputePipeline>(
        context, std::make_shared<Shader>(context, "render_st", SPV_RENDER_ST, SPV_RENDER_ST_len));
    auto inputSet = std::make_shared<DescriptorSet>(context, FRAMES_IN_FLIGHT);
    inputSet->bindBufferToDescriptorSet(0, vk::DescriptorType::eStorageBuffer, vk::ShaderStageFlagBits::eCompute,
        vertexAttributeBuffer);
    inputSet->bindBufferToDescriptorSet(1, vk::DescriptorType::eStorageBuffer, vk::ShaderStageFlagBits::eCompute,
    ...
    outputSet->build();
    renderPipeline_ST->addDescriptorSet(0, inputSet);
    renderPipeline_ST->addDescriptorSet(1, outputSet);
    renderPipeline_ST->addPushConstant(vk::ShaderStageFlagBits::eCompute, 0, sizeof(uint32_t) * 3);
    renderPipeline_ST->build();
}

bool RendererST::recordRenderCommandBuffer_ST(uint32_t currentFrame) {
    if (!renderCommandBuffer_ST) {
        renderCommandBuffer_ST = std::move(context->device->allocateCommandBuffersUnique(
            vk::CommandBufferAllocateInfo(commandPool.get(), vk::CommandBufferLevel::ePrimary, 1))[0]);
    }
    ...
    guiManager.pushTextMetric("instances", numInstances);
    auto [width, height] = swapchain->swapchainExtent;
    uint32_t tileX = (width + 16 - 1) / 16;
    uint32_t tileY = (height + 16 - 1) / 16;
    bool needsRealloc = false;
    if (numInstances > tileX * tileY * perTileSplatMultiplier) {
        auto old = perTileSplatMultiplier;
        while (numInstances > tileX * tileY * perTileSplatMultiplier) {
            perTileSplatMultiplier++;
        }
        spdlog::info("Reallocating instance buffers. {} -> {}", old, perTileSplatMultiplier);
        instanceIndexBuffer_ST->realloc(tileX * tileY * perTileSplatMultiplier * sizeof(uint32_t));
		needsRealloc = true;
    }
    ...
    renderCommandBuffer_ST->reset({});
    renderCommandBuffer_ST->begin(vk::CommandBufferBeginInfo{});
...
#endif
    // Synchronize vertex attribute buffer written in preprocess pass
    vertexAttributeBuffer->computeWriteReadBarrier(renderCommandBuffer_ST.get());
    // Clear instance counter buffer to 0
    renderCommandBuffer_ST->fillBuffer(instanceCounterBuffer_ST->buffer, 0, VK_WHOLE_SIZE, 0);
    Utils::BarrierBuilder().queueFamilyIndex(context->queues[VulkanContext::Queue::COMPUTE].queueFamily)
        .addBufferBarrier(instanceCounterBuffer_ST, vk::AccessFlagBits::eTransferWrite,
            vk::AccessFlagBits::eShaderWrite)
        .build(renderCommandBuffer_ST.get(), vk::PipelineStageFlagBits::eTransfer,
            vk::PipelineStageFlagBits::eComputeShader);
    auto numGroups = scene->getNumVertices();
    processInstancesPipeline_ST->bind(renderCommandBuffer_ST, 0, 0);
    renderCommandBuffer_ST->writeTimestamp(vk::PipelineStageFlagBits::eComputeShader, context->queryPool.get(),
        queryManager->registerQuery("process_instances_start"));
    renderCommandBuffer_ST->dispatch(numGroups, 1, 1);
    renderCommandBuffer_ST->writeTimestamp(vk::PipelineStageFlagBits::eComputeShader, context->queryPool.get(),
        queryManager->registerQuery("process_instances_end"));

    tileBoundaryPipeline_ST->bind(renderCommandBuffer_ST, 0, 0);
    uint32_t constants[3] = { width, height,uint32_t(spp)};
    renderCommandBuffer_ST->pushConstants(tileBoundaryPipeline_ST->pipelineLayout.get(),
        vk::ShaderStageFlagBits::eCompute, 0,
        sizeof(uint32_t) * 2, constants);
    renderCommandBuffer_ST->writeTimestamp(vk::PipelineStageFlagBits::eComputeShader, context->queryPool.get(),
        queryManager->registerQuery("tile_boundary_st_start"));
    renderCommandBuffer_ST->dispatch((tileX * tileY + 255) / 256, 1, 1);
    tileBoundaryBuffer_ST->computeWriteReadBarrier(renderCommandBuffer_ST.get());
    instanceIndexBuffer_ST->computeWriteReadBarrier(renderCommandBuffer_ST.get());
    instanceCounterBuffer_ST->computeWriteReadBarrier(renderCommandBuffer_ST.get());
    renderCommandBuffer_ST->writeTimestamp(vk::PipelineStageFlagBits::eComputeShader, context->queryPool.get(),
        queryManager->registerQuery("tile_boundary_st_end"));
    renderPipeline_ST->bind(renderCommandBuffer_ST, 0, std::vector<uint32_t>{0, currentImageIndex});
    renderCommandBuffer_ST->pushConstants(renderPipeline_ST->pipelineLayout.get(),
        vk::ShaderStageFlagBits::eCompute, 0, sizeof(uint32_t) * 3, constants);
    vk::ImageMemoryBarrier imageMemoryBarrier{};
    imageMemoryBarrier.oldLayout = vk::ImageLayout::eUndefined;
    imageMemoryBarrier.newLayout = vk::ImageLayout::eGeneral;
    imageMemoryBarrier.image = swapchain->swapchainImages[currentImageIndex]->image;
    ...
    imageMemoryBarrier.srcQueueFamilyIndex = VK_QUEUE_FAMILY_IGNORED;
    imageMemoryBarrier.dstQueueFamilyIndex = VK_QUEUE_FAMILY_IGNORED;
    renderCommandBuffer_ST->pipelineBarrier(vk::PipelineStageFlagBits::eTopOfPipe,
        vk::PipelineStageFlagBits::eComputeShader,
        vk::DependencyFlagBits::eByRegion, nullptr, nullptr, imageMemoryBarrier);
    renderCommandBuffer_ST->writeTimestamp(vk::PipelineStageFlagBits::eComputeShader, context->queryPool.get(),
        queryManager->registerQuery("render_start"));
    renderCommandBuffer_ST->dispatch((width + 15) / 16, (height + 15) / 16, 1);
    ...
    if (configuration.enableGui) {
        imageMemoryBarrier.newLayout = vk::ImageLayout::eColorAttachmentOptimal;
        imageMemoryBarrier.dstAccessMask = vk::AccessFlagBits::eColorAttachmentWrite;
        renderCommandBuffer_ST->pipelineBarrier(vk::PipelineStageFlagBits::eComputeShader,
            vk::PipelineStageFlagBits::eColorAttachmentOutput,
            vk::DependencyFlagBits::eByRegion, nullptr, nullptr, imageMemoryBarrier);
    }
    else {
    ...
    if (configuration.enableGui) {
        imguiManager->draw(renderCommandBuffer_ST.get(), currentImageIndex, std::bind(&GUIManager::buildGui, &guiManager));
        ...
        renderCommandBuffer_ST->pipelineBarrier(vk::PipelineStageFlagBits::eColorAttachmentOutput,
            vk::PipelineStageFlagBits::eComputeShader,
            vk::DependencyFlagBits::eByRegion, nullptr, nullptr, imageMemoryBarrier);
    }
    renderCommandBuffer_ST->end();
    return true;
}

void RendererST::recordPreprocessCommandBuffer_ST() {
    spdlog::debug("Recording preprocess command buffer ST");
    if (!preprocessCommandBuffer_ST) {
        vk::CommandBufferAllocateInfo allocateInfo = { commandPool.get(), vk::CommandBufferLevel::ePrimary, 1 };
        auto buffers = context->device->allocateCommandBuffersUnique(allocateInfo);
        preprocessCommandBuffer_ST = std::move(buffers[0]);
    }
    preprocessCommandBuffer_ST->reset();

    auto numGroups = (scene->getNumVertices() + 255) / 256;

    preprocessCommandBuffer_ST->begin(vk::CommandBufferBeginInfo{});

    preprocessCommandBuffer_ST->resetQueryPool(context->queryPool.get(), 0, 12);

    preprocessCommandBuffer_ST->fillBuffer(tileOverlapBuffer_ST->buffer, 0, VK_WHOLE_SIZE, 0);
    preprocessCommandBuffer_ST->fillBuffer(tileOverlapBuffer_ST->buffer, 0, VK_WHOLE_SIZE, 0);
    Utils::BarrierBuilder().queueFamilyIndex(context->queues[VulkanContext::Queue::COMPUTE].queueFamily)
        .addBufferBarrier(tileOverlapBuffer_ST, vk::AccessFlagBits::eTransferWrite,
            vk::AccessFlagBits::eShaderWrite)
        .build(preprocessCommandBuffer_ST.get(), vk::PipelineStageFlagBits::eTransfer,
            vk::PipelineStageFlagBits::eComputeShader);
    preprocessPipeline_ST->bind(preprocessCommandBuffer_ST, 0, 0);

    preprocessCommandBuffer_ST->writeTimestamp(vk::PipelineStageFlagBits::eComputeShader, context->queryPool.get(),
        queryManager->registerQuery("preprocess_start"));
    preprocessCommandBuffer_ST->dispatch(numGroups, 1, 1);


    tileOverlapBuffer_ST->computeWriteReadBarrier(preprocessCommandBuffer_ST.get());

    vk::BufferCopy copyRegion = { 0, 0, tileOverlapBuffer_ST->size };
    preprocessCommandBuffer_ST->copyBuffer(tileOverlapBuffer_ST->buffer, prefixSumPingBuffer->buffer, 1, &copyRegion);

    prefixSumPingBuffer->computeWriteReadBarrier(preprocessCommandBuffer_ST.get());

    preprocessCommandBuffer_ST->writeTimestamp(vk::PipelineStageFlagBits::eComputeShader, context->queryPool.get(),
        queryManager->registerQuery("preprocess_end"));

    prefixSumPipeline->bind(preprocessCommandBuffer_ST, 0, 0);
    preprocessCommandBuffer_ST->writeTimestamp(vk::PipelineStageFlagBits::eComputeShader, context->queryPool.get(),
        queryManager->registerQuery("prefix_sum_start"));
    auto [width, height] = swapchain->swapchainExtent;
    auto tileX = (width + 16 - 1) / 16;
    ...
    const auto iters = static_cast<uint32_t>(std::ceil(std::log2(static_cast<float>(tileX * tileY))));
    for (uint32_t timestep = 0; timestep <= iters; timestep++) {
        preprocessCommandBuffer_ST->pushConstants(prefixSumPipeline->pipelineLayout.get(),
            vk::ShaderStageFlagBits::eCompute, 0,
            sizeof(uint32_t), &timestep);
        preprocessCommandBuffer_ST->dispatch(numGroups, 1, 1);

        if (timestep % 2 == 0) {
            prefixSumPongBuffer->computeWriteReadBarrier(preprocessCommandBuffer_ST.get());
            prefixSumPingBuffer->computeReadWriteBarrier(preprocessCommandBuffer_ST.get());
        }
        else {
            prefixSumPingBuffer->computeWriteReadBarrier(preprocessCommandBuffer_ST.get());
            prefixSumPongBuffer->computeReadWriteBarrier(preprocessCommandBuffer_ST.get());
        }
    }
    ...
    if (iters % 2 == 0) {
        preprocessCommandBuffer_ST->copyBuffer(prefixSumPingBuffer->buffer, totalSumBufferHost->buffer, 1,
            &totalSumRegion);
        preprocessCommandBuffer_ST->copyBuffer(prefixSumPingBuffer->buffer, prefixBuffer_ST->buffer, 1,
            &prefixSumRegion);
    }
    else {
        preprocessCommandBuffer_ST->copyBuffer(prefixSumPongBuffer->buffer, totalSumBufferHost->buffer, 1,
            &totalSumRegion);
        preprocessCommandBuffer_ST->copyBuffer(prefixSumPongBuffer->buffer, prefixBuffer_ST->buffer, 1,
            &prefixSumRegion);
    }
    ...
    preprocessCommandBuffer_ST->end();
}