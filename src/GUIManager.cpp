...
void GUIManager::buildGui() {
    ...
    if (Owner) {
		Renderer::RendererType type = Owner->getRendererType();
        if(type == Renderer::RendererType::NormalSplatting) {
            
        } else if (type == Renderer::RendererType::StochasticSplatting) {
            ImGui::SetNextWindowPos(ImVec2(10, 450), ImGuiCond_FirstUseEver);
            ImGui::Begin("Stochastic Splatting", &popen, ImGuiWindowFlags_AlwaysAutoResize);
			ImGui::InputInt("SPP", &(static_cast<RendererST*>(Owner)->spp), 1, 32);
            ImGui::End();
		}
    }
    if (mouseCapture) {
